/**
 * components/icons/IconCollection.jsx
 * ─────────────────────────────────────
 * Single source of truth for every SVG icon used in the app.
 * Usage:
 *   import { Icon } from "@/components/icons/IconCollection";
 *   <Icon name="home" size={20} className="text-indigo-600" />
 *
 * Adding a new icon:
 *   1. Add an entry to the ICONS object below.
 *   2. Use it anywhere via <Icon name="yourKey" />.
 *
 * All icons use `currentColor` so they inherit CSS `color`.
 */
...
/**
 * Icon
 * @param {{ name: keyof ICONS, size?: number, strokeWidth?: number, className?: string }} props
 */
export const Icon = ({
  name,
  size = 20,
