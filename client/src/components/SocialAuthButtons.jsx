/**
 * components/auth/SocialAuthButtons.jsx
 * Renders Google / Facebook / Apple sign-in buttons.
 *
 * Props:
 *  - mode: "login" | "register"  (changes button label text)
 *  - disabled: bool              (disables all buttons — e.g. form is submitting)
 *  - className: string           (extra Tailwind classes for the wrapper)
 */

import React, { memo } from "react";
import { useSocialAuth } from "../hooks/useSocialAuth";

// ── SVG icons ─────────────────────────────────────────────────────────────

const GoogleIcon = () => (
  <svg className="w-5 h-5 shrink-0" viewBox="0 0 24 24" aria-hidden="true">
    <path
      d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
      fill="#4285F4"
    />
    <path
      d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
      fill="#34A853"
    />
    <path
      d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
      fill="#FBBC05"
    />
    <path
      d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
      fill="#EA4335"
    />
  </svg>
);

const FacebookIcon = () => (
  <svg className="w-5 h-5 shrink-0" viewBox="0 0 24 24" aria-hidden="true" fill="#1877F2">
    <path d="M24 12.073C24 5.405 18.627 0 12 0S0 5.405 0 12.073c0 6.026 4.388 11.02 10.125 11.927v-8.437H7.078v-3.49h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.49h-2.796v8.437C19.612 23.093 24 18.1 24 12.073z" />
  </svg>
);

const AppleIcon = () => (
  <svg className="w-5 h-5 shrink-0" viewBox="0 0 24 24" aria-hidden="true" fill="currentColor">
    <path d="M12.152 6.896c-.948 0-2.415-1.078-3.96-1.04-2.04.027-3.91 1.183-4.961 3.014-2.117 3.675-.54 9.103 1.519 12.09 1.013 1.454 2.208 3.09 3.792 3.039 1.52-.065 2.09-.987 3.935-.987 1.831 0 2.35.987 3.96.948 1.637-.026 2.676-1.48 3.676-2.948 1.156-1.688 1.636-3.325 1.662-3.415-.039-.013-3.182-1.221-3.22-4.857-.026-3.04 2.48-4.494 2.597-4.559-1.429-2.09-3.623-2.324-4.39-2.376-2-.156-3.675 1.09-4.61 1.09z" />
    <path d="M15.53 3.83c.843-1.012 1.4-2.427 1.245-3.83-1.207.052-2.662.805-3.532 1.818-.78.896-1.454 2.338-1.273 3.714 1.338.104 2.715-.688 3.56-1.701z" />
  </svg>
);

// ── Config ─────────────────────────────────────────────────────────────────

const SOCIAL_BUTTONS = [
  {
    key:        "google",
    label:      "Google",
    Icon:       GoogleIcon,
    baseClass:  "border-gray-200 bg-white text-gray-700 hover:bg-gray-50 hover:border-gray-300 focus-visible:ring-blue-500",
    loadingClass: "opacity-70 cursor-wait",
  },
  {
    key:        "facebook",
    label:      "Facebook",
    Icon:       FacebookIcon,
    baseClass:  "border-[#1877F2]/30 bg-[#1877F2]/5 text-[#1877F2] hover:bg-[#1877F2]/10 hover:border-[#1877F2]/50 focus-visible:ring-[#1877F2]",
    loadingClass: "opacity-70 cursor-wait",
  },
  {
    key:        "apple",
    label:      "Apple",
    Icon:       AppleIcon,
    baseClass:  "border-gray-900/20 bg-gray-900 text-white hover:bg-gray-800 focus-visible:ring-gray-700",
    loadingClass: "opacity-70 cursor-wait",
  },
];

// ── Spinner ────────────────────────────────────────────────────────────────

const Spinner = ({ color = "currentColor" }) => (
  <svg
    className="w-4 h-4 animate-spin shrink-0"
    xmlns="http://www.w3.org/2000/svg"
    fill="none"
    viewBox="0 0 24 24"
    aria-hidden="true"
  >
    <circle className="opacity-25" cx="12" cy="12" r="10" stroke={color} strokeWidth="4" />
    <path
      className="opacity-75"
      fill={color}
      d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4z"
    />
  </svg>
);

// ── Main component ─────────────────────────────────────────────────────────

const SocialAuthButtons = ({ mode = "login", disabled = false, className = "" }) => {
  const { handleSocialLogin, loadingProvider } = useSocialAuth();

  const actionLabel = mode === "register" ? "Sign up with" : "Continue with";
  const isAnyLoading = Boolean(loadingProvider);

  return (
    <div className={`flex flex-col gap-3 w-full ${className}`}>
      {SOCIAL_BUTTONS.map(({ key, label, Icon, baseClass, loadingClass }) => {
        const isThisLoading = loadingProvider === key;
        const isDisabled    = disabled || isAnyLoading;

        return (
          <button
            key={key}
            type="button"
            onClick={() => handleSocialLogin(key)}
            disabled={isDisabled}
            aria-label={`${actionLabel} ${label}`}
            aria-busy={isThisLoading}
            className={[
              // Base layout
              "relative flex items-center justify-center gap-3",
              "w-full px-4 py-2.5 rounded-xl",
              "text-sm font-medium",
              "border transition-all duration-150",
              "focus:outline-none focus-visible:ring-2 focus-visible:ring-offset-2",
              // Per-provider styles
              baseClass,
              // Loading / disabled
              isThisLoading ? loadingClass : "",
              isDisabled && !isThisLoading ? "opacity-50 cursor-not-allowed" : "",
            ]
              .filter(Boolean)
              .join(" ")}
          >
            {/* Icon or spinner */}
            {isThisLoading ? (
              <Spinner />
            ) : (
              <Icon />
            )}

            <span>
              {isThisLoading
                ? `Connecting to ${label}...`
                : `${actionLabel} ${label}`}
            </span>
          </button>
        );
      })}
    </div>
  );
};

export default memo(SocialAuthButtons);
